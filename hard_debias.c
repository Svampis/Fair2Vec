#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>

const long long max_w = 50;

int main(int argc, char **argv) {
    if (argc != 4) {
        printf("Usage: %s <input.bin> <word_list.txt> <output.bin>\n", argv[0]);
        return 1;
    }

    char *input_file = argv[1];
    char *words_file = argv[2];
    char *output_file = argv[3];

    FILE *f = fopen(input_file, "rb");
    if (!f) {
        printf("Input file not found.\n");
        return 1;
    }

    long long words, size;
    fscanf(f, "%lld", &words);
    fscanf(f, "%lld", &size);

    char *vocab = (char *)malloc(words * max_w * sizeof(char));
    float *M = (float *)malloc(words * size * sizeof(float));
    if (!vocab || !M) {
        printf("Cannot allocate memory.\n");
        return 1;
    }

    // Load vocabulary + vectors
    for (long long b = 0; b < words; b++) {
        int a = 0;
        while (1) {
            vocab[b * max_w + a] = fgetc(f);
            if (feof(f) || vocab[b * max_w + a] == ' ') break;
            if (a < max_w && vocab[b * max_w + a] != '\n') a++;
        }
        vocab[b * max_w + a] = 0;
        fread(&M[b * size], sizeof(float), size, f);
    }
    fclose(f);

    // Find "man" and "woman"
    long long idx_man = -1, idx_woman = -1;
    for (long long i = 0; i < words; i++) {
        if (!strcmp(&vocab[i * max_w], "man")) idx_man = i;
        if (!strcmp(&vocab[i * max_w], "woman")) idx_woman = i;
    }

    if (idx_man == -1 || idx_woman == -1) {
        printf("Error: 'man' or 'woman' not in vocabulary.\n");
        return 1;
    }

    // Compute gender direction = man - woman
    float *g = (float *)malloc(size * sizeof(float));
    for (long long i = 0; i < size; i++) {
        g[i] = M[idx_man * size + i] - M[idx_woman * size + i];
    }

    // Normalize gender direction
    float gnorm = 0;
    for (long long i = 0; i < size; i++) gnorm += g[i] * g[i];
    gnorm = sqrt(gnorm);
    for (long long i = 0; i < size; i++) g[i] /= gnorm;

    // Load list of words to debias
    FILE *wf = fopen(words_file, "r");
    if (!wf) {
        printf("Word list file not found.\n");
        return 1;
    }

    char buffer[256];
    while (fgets(buffer, sizeof(buffer), wf)) {
        buffer[strcspn(buffer, "\n")] = 0; // strip newline

        // find word in vocabulary
        long long idx = -1;
        for (long long i = 0; i < words; i++) {
            if (!strcmp(&vocab[i * max_w], buffer)) {
                idx = i;
                break;
            }
        }

        if (idx == -1) {
            printf("Skipping missing word: %s\n", buffer);
            continue;
        }

        float *v = &M[idx * size];

        // compute dot product v·g
        float dot = 0;
        for (long long j = 0; j < size; j++)
            dot += v[j] * g[j];

        // project away: v = v - dot * g
        for (long long j = 0; j < size; j++)
            v[j] -= dot * g[j];

        // renormalize vector
        float len = 0;
        for (long long j = 0; j < size; j++)
            len += v[j] * v[j];
        len = sqrt(len);
        for (long long j = 0; j < size; j++)
            v[j] /= len;

        printf("Debiased: %s\n", buffer);
    }

    fclose(wf);

    // Write output
    FILE *fo = fopen(output_file, "wb");
    fprintf(fo, "%lld %lld\n", words, size);
    for (long long i = 0; i < words; i++) {
        fprintf(fo, "%s ", &vocab[i * max_w]);
        fwrite(&M[i * size], sizeof(float), size, fo);
    }
    fclose(fo);

    free(vocab);
    free(M);
    free(g);

    printf("Done. Output saved to %s\n", output_file);
    return 0;
}

